...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if(rec->lock)
        strHTML += "<b>" + tr("Lock Status") + ":</b> <font color='red'><b>" + tr("Locked transaction. Unlock height is: ") + QString::number(rec->nUnlockHeight)+ "</b></font><br>";
    else
        strHTML += "<b>" + tr("Lock Status") + ":</b> <font color='red'><b>" + tr("Common transaction") + "</b><br>";

    if (wtx.IsCoinBase() || wtx.IsCoinStake())
    {
        quint32 numBlocksToMaturity = nStakeMinConfirmations;
        strHTML += "<br>" + tr("Generated coins must mature %1 blocks before they can be spent. When you generated this block, it was broadcast to the network to be added to the block chain. If it fails to get into the chain, its state will change to \"not accepted\" and it won't be spendable. This may occasionally happen if another node generates a block within a few seconds of yours.").arg(QString::number(nStakeMinConfirmations)) + "<br>";
    }
