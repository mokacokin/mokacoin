...
public:
    ...
    CLockData(int64_t nAmountIn, int nMonthIn)
    {
        nAmount = nAmountIn;
        nMonth = nMonthIn;
    }
...
public:
    ...
    bool CreateTransactionWithLock(const std::vector<std::pair<CScript, CLockData> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl *coinControl=NULL);
    bool CreateTransactionWithLock(CScript scriptPubKey, int64_t nLockAmount, int nLockMonth, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl *coinControl=NULL);
    ...
    int64_t GetUnlockReward(const int64_t nAmount, const int nMonth);
    int64_t GetUnlockedTime(const int64_t nStartTime, const int nMonth);
    std::string SendMoneyWithLock(CScript scriptPubKey, int64_t nLockAmount, int nLockMonth, CWalletTx& wtxNew, bool fAskFee=false);
    ...
    int64_t GetCredit(const CTxOut& txout, bool bContainLock = false) const
    {
        if (!MoneyRange(txout.nValue))
            throw std::runtime_error("CWallet::GetCredit() : value out of range");
        if(bContainLock)
            return (IsMine(txout) ? txout.nValue : 0);
        return (IsMine(txout) && !IsLockedTxOut(txout) ? txout.nValue : 0);
    }
    ...
    int64_t GetCredit(const CTransaction& tx, bool bContainLock = false) const
    {
        int64_t nCredit = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            nCredit += GetCredit(txout, bContainLock);
...
public:
    ...
    int64_t GetCredit(bool fUseCache=true, bool bContainLock = false) const
    {
        ...
        nCreditCached = pwallet->GetCredit(*this, bContainLock);
        fCreditCached = true;
        return nCreditCached;
    }

    int64_t GetAvailableCredit(bool bContainLock = false, bool fUseCache = true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                nCredit += pwallet->GetCredit(txout, bContainLock);
                if (!MoneyRange(nCredit))
